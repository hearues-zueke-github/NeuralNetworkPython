...
if __name__ == '__main__':
	...
	nl = [3, 5, 4, 2]
	l_bw = [np.random.uniform(-1, 1, (l1 + 1, l2)) for l1, l2 in zip(nl[:-1], nl[1:])]
	...
	x = np.random.uniform(-1, 1, (amount_values, nl[0]))
	# we have only one t value given, therefore t is a vector of length nl[-1] = 2
	t = np.random.randint(0, 2, (amount_values, nl[-1], )).astype(np.float64)

	def f_forward(x):
		arr_ones = np.ones((x.shape[0], 1))
		a = x
		for bw in l_bw[:-1]:
			a_1 = np.hstack((arr_ones, a))
			z = a_1.dot(bw)
			a = f_hidden(z)
	...
	def backprop(x, t):
		...
		l_nabla_bw[-1] = np.dot(np.hstack((arr_ones, l_y[-1-1])).transpose(), delta)

		for l in range(2, len(nl)):
			z = l_z[-l]
			sp = f_hidden_derv(z)
			delta = np.dot(delta, l_bw[-l+1][1:].transpose()) * sp
			l_nabla_bw[-l] = np.dot(np.hstack((arr_ones, l_y[-l-1])).transpose(), delta)
		return l_nabla_bw
